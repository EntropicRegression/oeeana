...
1. 按「1. 切段音檔」，只執行預處理、Whisper 配對及切段，不會載入 emotion2vec+ 或輸出 Excel。
2. 若訊息區列出缺失片段，依照顯示的完整路徑，將人工切割的 WAV 放入 `chopped/`。檔名格式為 `<原音檔檔名（不含副檔名）>_段落N.wav`，例如 `student01_段落3.wav`。WAV 的取樣率與聲道數可不同，分析時會統一解碼為 16 kHz 單聲道。
3. 所有缺失檔案補齊後，按「2. 開始情緒分析」。此階段只讀取 `chopped/` 中的切段 WAV，不會重新執行 Whisper、去雜音或覆蓋人工片段。

若全部片段原本就切割成功，可在第一階段結束後直接按第二個按鈕。程式重新啟動後，只要原輸入設定與 `chopped/` 仍存在，也可直接執行情緒分析。
...
分段流程會使用 `faster-whisper large-v3` 的 word timestamps、中文轉換（OpenCC）、中文比例與重複內容過濾、RapidFuzz 相似度、75% 動態門檻、15 秒最早候選群組及 1 秒前後緩衝，並將預處理後的切段 WAV 快取在來源音檔旁的 `chopped/`。快取 manifest 會記錄來源檔案、文字、分段參數與去雜音 profile；其中任一項改變時，重新按下切段按鈕會更新自動切割結果。人工補入片段後應直接進行情緒分析，避免再次執行切段而覆蓋同名檔案。
