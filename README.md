...
介面參數：標準音檔、分段文字檔、需分析的檔案資料夾與 Whisper 相似度門檻。分段文字檔需為 UTF-8；選取檔案後會自動解析並填入段落數量。

「重置介面」會清空各頁籤的輸入、表格、進度與訊息，並將 Whisper 門檻恢復為 `0.30`；不會刪除原始音檔、`chopped/` 切段快取或已輸出的 Excel。
...
分段流程會使用 `faster-whisper large-v3` 的 word timestamps、中文轉換（OpenCC）、中文比例與重複內容過濾、RapidFuzz 相似度、75% 動態門檻、15 秒最早候選群組及 1 秒前後緩衝，並將切段 WAV 快取在來源音檔旁的 `chopped/`。
